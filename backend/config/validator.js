...
const validator = (req, res, next) => {
    const schema = joi.object({
        firstName: joi.string().max(20).min(3).trim().pattern(new RegExp('[a-zA-Z]')).required().message({
            'string.min': 'The NAME must contain more than 3 characters',
            'string.max':'The name must contain a maximum of 20 characters'
        }),

        lastName: joi.string().max(20).min(3).trim().pattern(new RegExp('[a-zA-Z]')).required().message({
